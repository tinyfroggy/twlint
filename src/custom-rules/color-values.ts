 ...
 * theme's tokens. OKLab is perceptually uniform enough that a plain Euclidean
 * distance says whether two colors are "the same gray" or "the same red".
 */
...
export function isNamedColor(value: string): boolean {
  return Object.hasOwn(NAMED_COLORS, value.trim().toLowerCase());
}
...
function fromRgb(r: number, g: number, b: number): Lab {
  const lr = linear(r);
  const lg = linear(g);
  const lb = linear(b);
...
function fromHsl(h: number, s: number, l: number): Lab {
  return fromRgb(...fromHslRgb(h, s, l));
}

function fromHwb(h: number, w: number, b: number): Lab {
  if (w + b >= 1) {
    const gray = w / (w + b);
    return fromRgb(gray, gray, gray);
  }
  const [r, g, bl] = fromHslRgb(h, 1, 0.5);
  const scale = (c: number) => c * (1 - w - b) + w;
  return fromRgb(scale(r), scale(g), scale(bl));
}

function fromHslRgb(h: number, s: number, l: number): [number, number, number] {
...
/** Color-function arguments, with the alpha channel dropped. */
...
function channel(raw: string | undefined, scale: number, percentScale = 1): number | null {
  if (raw === undefined) return null;
  // CSS `none` is a missing channel, computed as zero.
...
/** Parse a literal CSS color into OKLab. Null for variables, keywords, etc. */
export function parseColor(value: string): Lab | null {
  ...
  if (fn === "color") {
    if (!inner.trim().startsWith("srgb")) return null;
    const parts = args(inner.trim().replace(/^srgb\s+/, "")).map((p) => channel(p, 1));
    const [r, g, b] = parts;
...
/** Tie order among tokens that share a color value. */
const PRIORITY = [
  "background",
  "foreground",
  "muted",
...
export function roleOf(prefix: string): ColorRole {
  return /^(?:text|placeholder|caret|decoration|fill|stroke)$/.test(prefix) ? "text" : "surface";
}

const isForeground = (name: string) => name === "foreground" || name.endsWith("-foreground");

/**
 * Nearest theme tokens for a color, collapsed by equal value and ordered by
 * role: text prefers `-foreground` tokens, surfaces avoid them.
 */
export function nearestColorTokens(
  lab: Lab,
  tokens: Map<string, Lab>,
  role: ColorRole,
  limit = 2,
): string[] {
  const groups = new Map<string, { distance: number; names: string[] }>();
  ...
  const prefer = role === "text" ? isForeground : (name: string) => !isForeground(name);
  ...
  if (role === "surface" && candidates.some((name) => !isForeground(name))) {
    candidates = candidates.filter((name) => !isForeground(name));
  }
